...
def occultquad_interp(z,p,u1,u2, func):
    #sm=z<(1.0+p)
    #ret=np.ones(len(z))
    #order_ind=np.argsort(z)#z[sm]
    #order_ind_2=np.argsort(order_ind)
    order_ind_2=np.argsort(np.argsort(z))
    #zm_ord=z[sm][order_ind]
    #ret[sm] = (1.0 + (func[0](zm_ord,p)+u1*func[1](zm_ord,p)+u2*func[2](zm_ord,p))/(1.0-u1/3.0-u2/6.0))[order_ind_2]
    #return ret
    return (1.0 + (func[0](z,p)+u1*func[1](z,p)+u2*func[2](z,p))/(1.0-u1/3.0-u2/6.0))[order_ind_2]



def generate_occultquad_functions(z_bounds, p_bounds,z_res,p_res):

    z_grid=np.linspace(z_bounds[0],z_bounds[1],z_res)
    p_grid=np.linspace(p_bounds[0],p_bounds[1],p_res)
    ...
    long_o2=[]
    for p in p_grid:
        long_p.append([p]*len(z_grid))
        long_z.append(list(z_grid))
        m = occultquad_point(z_grid,p)
        grid_o0.append(m[0])
        grid_o1.append(m[1])
        grid_o2.append(m[2])
        long_o0.append(list(m[0]))
        long_o1.append(list(m[1]))
        long_o2.append(list(m[2]))
    grid_o0=np.array(grid_o0)
    grid_o1=np.array(grid_o1)
    grid_o2=np.array(grid_o2)

    funcs=[]

    from scipy.interpolate import interp2d as interpolation_method
    for grid in [grid_o0,grid_o1,grid_o2]:
        f=interpolation_method(z_grid,p_grid,grid)
        #pl.pcolor(z_grid,p_grid,f(z_grid,p_grid))
        #pl.show()
        funcs.append(f)
    return funcs
